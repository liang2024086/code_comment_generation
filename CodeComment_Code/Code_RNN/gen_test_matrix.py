...
def _read_parse_tree(fileName,wether_comment=0):
    ...
    self_vocabulary =_read_identifier_index('./rename_data/vocabularyMap.txt')
    self_inter_type = _read_inter_type_index('./rename_data/interMap.txt')
    ...
    for i in range (1,len(lines)):
	    ...
	    if (newNums[3] == 0):
                widthCount += 1
		if vocabulary.has_key(self_vocabulary[newNums[2]]):
		    print 'AAA ' , newNums[2],
		    newNums[2] = vocabulary[self_vocabulary[newNums[2]]]
		    print newNums[2]
		else:
		    print 'BBB '+self_vocabulary[newNums[2]],newNums[2],
		    newNums[2] = vocabulary['UNK']
		    print newNums[2]

            if (newNums[3] > 0):
		if inter_type.has_key(self_inter_type[newNums[2]]):
		    print 'CCC ' , newNums[2],
		    newNums[2] = inter_type[self_inter_type[newNums[2]]]
		    print  newNums[2]
		else:
		    newNums[2] = vocabulary['UNK']
		    print 'DDD '+self_vocabulary[newNums[2]]
...
def convert_method2method_name(parse_trees):
    ...
    vocabulary =_read_identifier('./data/vocabularyMap.txt')
    inter_type = _read_inter_type('./data/interMap.txt')
    method_index2name_map = _read_method_map('./rename_data/methodMap.txt')
...
#    methodComments,maxLength = _read_comments('data/methodCommentMap.txt',wordToIndex)
    ...
    if wether_train:
        which_cluster = np.array(gen_raw_method_cluster('./data/methodMap.txt',num_methods))
    else:
        which_cluster = np.array(gen_raw_method_cluster('./rename_data/methodMap.txt',num_methods))
...
#    which_cluster = gen_raw_method_cluster('./data/methodMap.txt')
#    print which_cluster
